...
전역변수란 자바스크립트에서 제일 바깥 범위(window 객체)에 변수를 만드는 것이다.

#####문제점

협업, 외부 라이브러리, 비동기 처리시 실행 순서를 알기 힘들어 충돌 위험이 있다.

#####해결

1. 한 번 함수 안에 넣어 지역변수로 만드는 것이다.
2. 네임스페이스를 달아준다. ( = 객체 안의 속성으로 만든다)
...
> 스코프 때문에 지역변수 x 는 전역변수 x에 영향을 끼칠 수 없다.
```js
var x = 'global';
function ex() {
  var x = 'local';
...
> js는 스코프를 점차 넓혀가며 찾는다. 꼬리를 물고 계속 스코프 범위를 넓히며 찾는 관계를 스코프 체인이라 부른다.

 자신 스코프 -> outer 스코프 -> ... -> 전역 스코프
...
> 함수를 처음 선언하는 순간, 함수 내부의 변수는 자기 스코프로부터 가장 가까운 곳에 있는 변수를 계속 참조하게 됩다.
...
결과 : nero, hello zero // 렉시컬 스코핑에 의함.
```

#### Private

> 내부 함수에서는 외부 함수의 변수에 접근 가능하다.
> 외부 함수에서는 내부 함수의 변수에 접근할 수 없다.

**외부함수가 소멸된 이후에도, 내부 함수가 외부함수에 접근 가능한게 클로저다???**

obj 네임스페이스의 x,y값은 고의적으로 누군가에게 변경 당하기 쉽다.
```js
// obj라는 이름의 네임스페이스
var obj = {
  x: 'local',
...
**x라는 변수에 바로 접근할 수 없도록 함수로 감싸 스코프를 생성한다.
함수로 감싸안았기 때문에, 외부에서 접근 가능해야 되는 y는 return한다.** 

```js
//newScope라는 이름의 네임스페이스
...
1. Function **선언**, with, catch 만 스코프를 생성한다.
...
>언제 끝날 지 모르는 동작(listen)에 대해 그 결과를 전달받을 때.
